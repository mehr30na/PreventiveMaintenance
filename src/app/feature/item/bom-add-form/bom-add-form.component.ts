...
@Component({
  selector: 'bom-add-form',
  templateUrl: './bom-add-form.component.html',
  styleUrls: ['./bom-add-form.component.scss']
})
export class BomAddFormComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.partService.create(this.part).subscribe((res:Part)=>{
      if(res){
        if(this.target=='asset'){
          this.partData.setPartOne(res);
        }else if(this.target=='workOrder'){
          this.partData.setPartOneWO(res);
        }
      }
    })
