...
(function ($) {
    ...
    function getTodayCalendarInPersian() {
        ...
        //return [persianDate.jy, persianDate.jm, persianDate.jd, getPersianWeekDayNameWithEnglishIndex(todayHeader.getDay())];
    ...
    function createPersianDateTimePickerHtml($popoverDescriber, dateTimeInJsonFormat, writeDateString, initializing) {
        var triggerName = $popoverDescriber.attr('data-trigger'),
            ...
            $calendarFooter = $('<tfoot><tr><td colspan="100"><a class="" href="javascript:void(0);" data-name="go-todayHeader">' + todayDateTimeString + '</a></td></tr></tfoot>'),
        ...
        for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
            ...
            if (currentLoopDateNumber == todayDateNumber) {
                $td.addClass('bg-primary').attr('data-name', 'todayHeader');
        ...
        $('[data-name="md-datetimepicker-name"]').html(dayOfWeek + '، ' + toPersianNumber(zeroPad(dateTimeInJsonFormat.Day)) + ' ' + persianMonthName + ' ' + toPersianNumber(zeroPad(dateTimeInJsonFormat.Year)));
    ...
    function createGregorianDateTimePickerHtml($popoverDescriber, dateTimeInJsonFormat, writeDateString, initializing) {
        ...
        // عوض کردن عنوان popover
        $('[data-name="md-datetimepicker-name"]').html(dayOfWeek + '، ' +
    ...
    function bindEvents() {
        // کلیک روی روزها
        $(document).on('click', '[data-name="day"],[data-name="todayHeader"]', function () {
        ...
        // کلیک روی دکمه امروز
        $(document).on('click', '[data-name="go-todayHeader"]', function () {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                $this.popover({
                    ...
                    title: 'انتخاب تاریخ',
                    trigger: 'manual',
                    template: '<div class="popover zIndexCorrected" role="tooltip"><div class="arrow"></div><h3 class="popover-name" data-name="md-datetimepicker-name" data-isgregorian="' + settings.IsGregorian + '"></h3><div class="popover-content" data-name="md-datetimepicker-popovercontent"></div></div>'
