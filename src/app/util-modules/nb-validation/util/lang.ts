...
// -----------------------------
export function addValidationEl(dest, vClassSuffix, innerHtml, ValidationConfig: ValidationConfig) {
  deleteElementByClassName(dest + vClassSuffix);
  document.getElementById(dest).appendChild(
    getMyValidationElement(dest, vClassSuffix, innerHtml, ValidationConfig));
}
export function delValidationEl(dest, vClassSuffix) {
  deleteElementByClassName(dest + vClassSuffix);
}
export function getMyValidationElement(dest, vClassSuffix, innerHtml, ValidationConfig: ValidationConfig) {
  const myEl = ValidationConfig.getValidationAlertElement(dest + vClassSuffix);
...
export function manageAddValidationAlert(dest, vClassSuffix, innerHTML, ValidationConfig: ValidationConfig) {
  if (!isNullOrUndefined(dest)) {
    addValidationEl(dest, vClassSuffix, innerHTML, ValidationConfig);
  }
}
// -----------------------------
